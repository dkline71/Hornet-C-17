...
void node::add_ingress(node_id src, boost::shared_ptr<ingress> ingress)   {
...
void node::add_egress(node_id dst, boost::shared_ptr<egress> egress)   {
...
boost::shared_ptr<ingress> node::get_ingress_from(node_id src)   {
    assert(src.is_valid());
    if (ingresses.find(src) == ingresses.end()) {
...
boost::shared_ptr<egress> node::get_egress_to(node_id dst)   {
    assert(dst.is_valid());
    if (egresses.find(dst) == egresses.end()) {
